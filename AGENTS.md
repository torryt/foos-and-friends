...
- **ELO (all-time)**: separate from seasonal — a continuous replay of the group's full match history as if seasons never reset, symmetric K=32, no clamp. Implemented twice: `replayContinuousElo` in `packages/shared/src/utils/elo.ts` and `compute_group_global_rankings` in `database/migrations/001_initial_schema.sql` — they must stay in sync.
...
- **Roles**: `GroupRole = 'owner' | 'admin' | 'member'`. Owners and admins manage members (list/promote/remove) via RPCs (`get_group_members`, `promote_group_member`, etc.); season management stays owner-only.
