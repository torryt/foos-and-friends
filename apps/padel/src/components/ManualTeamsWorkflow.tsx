...
const SlotButton = ({ position, player, onClick }: SlotButtonProps) => (
  <button
    type="button"
    onClick={onClick}
    className="w-full flex items-center justify-between gap-2 p-3 min-h-12 rounded-lg border border-gray-200 bg-white hover:bg-gray-50 text-left transition-colors"
  >
    <span className="flex items-center gap-2 truncate">
      {position === 'attacker' ? (
        <Sword className="text-orange-500 shrink-0" size={16} />
      ) : (
...
export const ManualTeamsWorkflow = ({
  ...
  return (
    <ModalOrBottomDrawer onClose={onClose} className="sm:max-w-md">
      <div className="bg-white w-full shadow-2xl border border-gray-100 flex flex-col max-h-full sm:max-h-[90vh]">
        <div
