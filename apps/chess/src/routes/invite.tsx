...
function InvitePageComponent() {
  ...
  if (checkingMembership && isAuthenticated) {
    return (
      <div className="flex items-center justify-center min-h-[60vh] p-4">
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-8 max-w-md shadow-2xl border border-white/50 text-center">
          <Loader className="animate-spin mx-auto mb-4 text-[#832161]" size={48} />
      ...
      <div className="flex items-center justify-center min-h-[60vh] p-4">
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-8 max-w-md shadow-2xl border border-white/50 text-center">
          ...
          <div className="flex items-center justify-center">
            <Loader className="animate-spin text-[#832161]" size={24} />
          </div>
        </div>
      </div>
    )
    ...
    <div className="flex items-center justify-center min-h-[60vh] p-4">
      <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-8 max-w-md shadow-2xl border border-white/50">
        <div className="text-center mb-8">
          <div className="w-16 h-16 bg-gradient-to-r from-[#832161]/20 to-[#DA4167]/20 rounded-full flex items-center justify-center mx-auto mb-4">
            <Users className="text-[#832161]" size={32} />
          </div>
          <h1 className="text-2xl font-bold text-gray-900 mb-2">You're Invited!</h1>
          <p className="text-gray-600">
            {groupInfo?.name ? `Join "${groupInfo.name}"` : 'Join this chess group'} and start
        ...
        <div className="space-y-4">
          <button
            ...
            disabled={loading}
            className="w-full bg-gradient-to-r from-[#832161] to-[#DA4167] text-white px-6 py-3 rounded-lg font-semibold hover:from-[#6e1b52] hover:to-[#c93558] transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            {loading ? (
              <>
