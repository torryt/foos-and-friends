-- Migration: Add chess sport type
-- This extends the sport_type constraint to include chess

-- Drop the existing constraint
ALTER TABLE friend_groups
  DROP CONSTRAINT IF EXISTS friend_groups_sport_type_check;

-- Add the new constraint with chess included
ALTER TABLE friend_groups
  ADD CONSTRAINT friend_groups_sport_type_check
  CHECK (sport_type IN ('foosball', 'padel', 'chess'));

-- Update the create_group_with_membership function to accept chess sport_type
...
BEGIN
  ...
  -- Validate sport_type (now includes chess)
